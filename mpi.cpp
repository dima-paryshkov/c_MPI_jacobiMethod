...
int main(int argc, char **argv)
{
    ...
    // В результате выполнения этой функции создается группа процессов, в которую помещаются
    // все процессы приложения, и создается область связи, описываемая предопределенным коммуникатором
    // MPI_COMM_WORLD. Эта область связи объединяет все процессы-приложения. Процессы в группе упорядочены
    // и пронумерованы от 0 до groupsize-1, где groupsize равно числу процессов в группе. Кроме этого,
    // создается предопределенный коммуникатор MPI_COMM_SELF, описывающий свою область связи для каждого отдельного процесса.
    MPI_Init(&argc, &argv);
    ...
    if (procCount > (Nx - 2))
    {
        ...
        // Завершает все процессы MPI, связанные с коммуникатором MPI_COMM_WORLD
    ...
    if (myRank == MAIN_PROC_RANK)
    {
        ...
        for (int destRank = 1; destRank < procCount; destRank++)
        {
            ...
            // tag процессу dest в области связи коммуникатора comm. Переменная buf - это, как правило, 
            // массив или скалярная переменная.
    ...
    for (int i = 0; i < procCount; i++)
    {
        ...
        // Блокирует вызывающего абонента до тех пор, пока его не вызовут все процессы в коммуникаторе; 
        // то есть вызов возвращается в любой процесс только после того, как все члены коммуникатора вошли в вызов.
        MPI_Barrier(MPI_COMM_WORLD);
    }
